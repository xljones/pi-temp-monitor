...
class CursesBarGraph:
    ...
    def update(self, values) -> None:
        ...
        self._window.erase()
        for column, v in enumerate(self._averaged_chunks(values, per_bucket)):
            assert column < w
            self._draw_bar(column, v, h)

        self._draw_axis_labels(h, w, column, len(values))
        self._window.refresh()

    def _averaged_chunks(self, iterable, n) -> Iterator[float]:
        summed_v = 0
        summed_count = 0
        for v in iterable:
            summed_v += v
    ...
    def _draw_bar(self, column, value, h) -> None:
        bar_len = max(0, min(h - 1, int(h * (value / self._max))))
        # vline draws from the starting coordinate towards positive y (down).
        self._window.vline((h - 1) - bar_len, column, ord("|"), bar_len)

    def _draw_axis_labels(self, h, w, max_column, num_values) -> None:
